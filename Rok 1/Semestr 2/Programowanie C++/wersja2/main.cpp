...
/*
	...
	Temat 2: typ string, losowe generowanie danych srodkami jezyka c++, implementacja agregacji (zad. dom),
	uwzglednic w programie mozliwosc wprowadzenia danych z klawiatury (getline) do celu przypisania wartosci atrybutom wybranego
	obiektu. np 850; 2015; Boeing 777; LOT, sprawdzamy typy przy wpisywaniu
*/
...
void zad5(Auto*& tablica, int& arraySize) {
	...
	*/
	...
	cout << "\n    Podaj marke, numer rejestracyjny i ilosc koni oddzielona znakiem ';' przyklad:" << endl;
	cout << "\n    Skoda;ZST 16161;70" << endl;
	...
	if (typeid(seglist[0]) == typeid(string)) {
		newArray[arraySize].marka = seglist[0];
	}
	if (typeid(seglist[1]) == typeid(string)) {
		newArray[arraySize].numerRejestracyjny = seglist[1];
	}
	if (typeid(stoi(seglist[2])) == typeid(int)) {
		newArray[arraySize].iloscKoni = stoi(seglist[2]);
	}
...
// funkcja, ktora pozwala na edycje dodanego auta
// ZAD 6
void zad6(Auto*& tablica, int& arraySize) {
	...
	if (flag == false) {
		...
		if (typeid(seglist[0]) == typeid(string)) {
			tablica[indexToEdit].marka = seglist[0];
		}
		if (typeid(seglist[1]) == typeid(string)) {
			tablica[indexToEdit].numerRejestracyjny = seglist[1];
		}
		if (typeid(stoi(seglist[2])) == typeid(int)) {
			tablica[indexToEdit].iloscKoni = stoi(seglist[2]);
		}

		cout << "\n========================================\n\n";
		cout << "    Auto z indeksem: " << indexToEdit << " zostało zaktualizowane\n\n";
		cout << "    Nacisnij ENTER by wrócić do menu\n";
		cout << "\n========================================\n";
		cin.get();
	}
...
// ZAD 8
void zad8(Auto*& tablica, int& arraySize) {
...
void menu(Auto*& tablica, Auto**& tablicaWskaznikow, int& arraySize, int& arraySizeWskaznikow) {
	...
	do {
		...
		cout << "  6] Edytuj wybrane Auto\n";
		cout << "  7] Usuń losowo wybrane Auto\n";
		cout << "  8] Wyszukaj auta od ilości koni, by je polubić\n";
		cout << "  9] Gra w pościg\n";
		cout << " 10] Gra w snake\n";
		cout << "  0] Wyjdź\n\n";
...
int main() {
	...
	Ogloszenia ogloszenia{ tablica };
	//Auto* tablica{ nullptr };
	//Auto** tablicaWskaznikow{ nullptr };
	// tablica wskaznikow
	...
	while (true) {
		menu(ogloszenia.auta, tablicaWskaznikow, arraySize, arraySizeWskaznikow);
	}
}