...
def run_experiment(structure, activation_name, algo_name, learning_rate):
    structure_dir = f"results/{structure}"
    activation_dir = os.path.join(structure_dir, activation_name)
    algo_dir = os.path.join(activation_dir, algo_name)
    lr_dir = os.path.join(algo_dir, f"{learning_rate:.1e}")
    ...
    approx = MLPApproximator(structure=structure, activation_name=activation_name, targets_activation_name="linear",
                             ...
                             algo_name=algo_name, learning_rate=learning_rate,
                             n_epochs=1000, batch_size=10, seed=53828,
    ...
    with open(os.path.join(lr_dir, "neural_network.pkl"), "wb") as f:
    ...
    metrics = (f"LOSS TRAIN (MSE): {mse}\n"
               f"LOSS TEST (MSE): {mse_test}\n"
               f"R^2 TRAIN: {approx.score(X_train, y_train)}\n"
               f"R^2 TEST: {approx.score(X_test, y_test)}\n")
    save_results(lr_dir, "metrics.txt", metrics)
    ...
    ax_target.plot_surface(X1, X2, Y_ref, cmap=mpl.colormaps["Spectral"])
    ax_approximator.set_title("NEURAL APPROXIMATOR")
    ax_approximator.plot_surface(X1, X2, Y_pred, cmap=mpl.colormaps["Spectral"])
    ...
    plot_filename = os.path.join(lr_dir, "results_plot.png")
...
if __name__ == '__main__':
    ...
    total_experiments = len(structures) * len(activations) * len(algorithms) * len(learning_rates)
        ...
        for activation_name in activations:
            for algo_name in algorithms:
                for learning_rate in learning_rates:
                    run_experiment(structure, activation_name, algo_name, learning_rate)

                    completed_experiments += 1
                    remaining_experiments = total_experiments - completed_experiments
                    print(f"\nCompleted: {completed_experiments}/{total_experiments} | Remaining: {remaining_experiments}\n")
